...
from atria_ml.training.engines._test_engine import TestEngine
...
from atria_insights.feature_perturbation._config import FeaturePerturbationConfig
...
@dataclass
class FeaturePerturbationEvaluatorState:
    data_pipeline: DataPipeline
    feat_pert_eval_pipeline: ExplainableModelPipeline
...
class FeaturePerturbationEvaluator:
    def __init__(
        self,
        config: FeaturePerturbationConfig,
        local_rank: int = 0,
        ...
        self._state: FeaturePerturbationEvaluatorState = self._build(
    ...
    def _build_explanation_engine(
        ...
        return ExplanationEngine(
            ...
            deps=ExplanationEngineDependencies(
                model_pipeline=self._state.feat_pert_eval_pipeline._model_pipeline,
                feat_pert_eval_pipeline=self._state.feat_pert_eval_pipeline,
    ...
    def _build(self, local_rank: int = 0) -> FeaturePerturbationEvaluatorState:
        ...
        feat_pert_eval_pipeline = self._config.feat_pert_eval_pipeline.build(
            labels=labels, persist_to_disk=True, cache_dir=self._run_dir
        )

        # log model pipeline
        logger.info(feat_pert_eval_pipeline.summarize())

        # get model transforms
        train_transform = feat_pert_eval_pipeline.config.model_pipeline.train_transform
        eval_transform = feat_pert_eval_pipeline.config.model_pipeline.eval_transform
        ...
        return FeaturePerturbationEvaluatorState(
            data_pipeline=data_pipeline,
            feat_pert_eval_pipeline=feat_pert_eval_pipeline,
            tb_logger=tb_logger,
        )
    ...
    def _build_test_engine(self) -> TestEngine:
        ...
        return FeaturePerturbationEvaluatorEngine(
            config=FeaturePerturbationEvaluatorEngineConfig(
                logging=self._config.logging,
                test_run=self._config.test_run,
                use_fixed_batch_iterator=self._config.use_fixed_batch_iterator,
            ...
            deps=FeaturePerturbationEvaluatorEngineDependencies(
                model_pipeline=self._state.feat_pert_eval_pipeline._model_pipeline,
                dataloader=test_dataloader,
                device=torch.device(self._device),
                output_dir=self._run_dir,
