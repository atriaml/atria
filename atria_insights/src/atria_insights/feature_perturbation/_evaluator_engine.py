...
class RobustnessEvalEngineConfig(EngineConfig):
    enable_outputs_caching: bool = True
    compute_metrics: bool = True


class RobustnessEvalEngineDependencies(EngineDependencies):
    x_model_pipeline: ExplainableModelPipeline


class RobustnessEvalEngine(
    EngineBase[RobustnessEvalEngineConfig, RobustnessEvalEngineDependencies]
):
    def __init__(
        self, config: RobustnessEvalEngineConfig, deps: RobustnessEvalEngineDependencies
    ) -> None:
        self._config = config
        self._deps = deps
