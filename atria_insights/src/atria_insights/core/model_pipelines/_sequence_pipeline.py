...
from atria_insights.core.model_pipelines.baseline_generators._sequence import (
...
class ExplainableSequenceModelPipelineConfig(ExplainableModelPipelineConfig):
    ...
    def validate_configs(self) -> ExplainableSequenceModelPipelineConfig:
        if not isinstance(
            self.feature_segmentor_config, SequenceFeatureMaskSegmentorConfig
        ):
            raise ValueError(
                "feature_segmentor_config must be an instance of SequenceFeatureMaskSegmentorConfig"
            )
        if not isinstance(
            self.baseline_generator_config, SequenceBaselineGeneratorConfig
        ):
            raise ValueError(
                "baseline_generator_config must be an instance of SequenceBaselineGeneratorConfig"
            )
        return self
