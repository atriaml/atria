...
class MetricsCacher:
    ...
    def save_metrics(
        self, metrics: Mapping[str, Any], sample_keys: list[str], metric_key: str
    ) -> None:
        # flatten the dict of metrics
        metrics = _flatten_dict(metrics)

        with HFSampleSaver(self._get_metric_path(metric_key)) as hfio:
            for metric_param_name, metrics_batch in metrics.items():
                ...
                for sample_key, metric_param_value in zip(sample_keys, metrics_batch):
                    ...
                    hfio.save(metric_param_name, metric_param_value, sample_key)

    def load_metrics(self, metric_key: str, sample_keys: list[str]) -> None:
        metric_file_path = self._get_metric_path(metric_key)
        ...
        with HFSampleSaver(metric_file_path, mode="r") as hfio:
            ...
            for sample_key in sample_keys:
                ...
                loaded_metrics = {}
                for key in keys:
                    loaded_metrics[key] = torch.from_numpy(hfio.load(key, sample_key))
