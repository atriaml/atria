...
class ExplanationPipeline(ABC, ExplainerPipelineConfigMixin):
    ...
    def build(
        ...
        self._built_explainer = self.config.explainer(
            model=self._wrap_model(self._model_pipeline.model)
        )
        if self.config.explainer_metrics is None:
            ...
            self._built_explainer_metrics = {
                metric_name: TorchXAIMetric(
                    ...
                    forward_func=self._wrap_model(self._model_pipeline.model),
                    device=device,
