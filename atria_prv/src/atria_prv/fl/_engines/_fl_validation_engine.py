...
from atria_prv.fl._engines._fl_training_engine_child import (
    FLTrainingChildEngine,
    FLTrainingChildEngineConfig,
    FLTrainingChildEngineDependencies,
)
...
class FLValidationEngineDependencies(FLTrainingChildEngineDependencies):
    pass


class FLValidationEngineConfig(FLTrainingChildEngineConfig):
    early_stopping: EarlyStoppingConfig = EarlyStoppingConfig()
    model_checkpoint: ModelCheckpointConfig = ModelCheckpointConfig()


class FLValidationEngine(
    FLTrainingChildEngine[FLValidationEngineConfig, FLValidationEngineDependencies]
):
    def __init__(
        self, config: FLValidationEngineConfig, deps: FLValidationEngineDependencies
    ):
    ...
    def _build_engine_step(self) -> EngineStep:
        assert self._deps.training_engine is not None, (
            "Parent engine must be set for FLValidationEngine."
        )
    ...
    def attach_metrics_output_file(self) -> None:
        ...
        def log_metrics_to_file(engine: Engine) -> None:
            ...
            all_metrics[f"round_{parent_epoch}"] = metrics
