...
from atria_prv.fl._engines._fl_training_engine import FLTrainingEngine
...
class FLTrainingChildEngineDependencies(EngineDependencies):
    training_engine: FLTrainingEngine


class FLTrainingChildEngineConfig(EngineConfig):
    run_every_n_epochs: float = 1.0
    run_on_start: bool = True
    use_ema: bool = False


T_FLTrainingChildEngineConfig = TypeVar(
    "T_FLTrainingChildEngineConfig", bound=FLTrainingChildEngineConfig
)
T_FLTrainingChildEngineDependencies = TypeVar(
    "T_FLTrainingChildEngineDependencies", bound=FLTrainingChildEngineDependencies
)


class FLTrainingChildEngine(
    EngineBase[T_FLTrainingChildEngineConfig, T_FLTrainingChildEngineDependencies]
):
    def __init__(
        self,
        config: T_FLTrainingChildEngineConfig,
        deps: T_FLTrainingChildEngineDependencies,
    ):
    ...
    def run(self) -> State | None:  # type: ignore[override]
        ...
        if self._deps.output_dir is not None:
            logger.info(
                f"Running {self.__class__.__name__} engine on [{len(self._deps.dataloader.dataset)}] samples with batch size [{self._deps.dataloader.batch_size}] and output_dir: {self._deps.output_dir}"
            )
        else:
