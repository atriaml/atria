...
def _output_transform(output: LayoutTokenClassificationModelOutput):
    ...
    logits_bs, target_bs, bbox_bs = (
        _get_bsz(output.logits),
        _get_bsz(output.target_label_values),
        _get_bsz(output.word_bboxes),
    )
    assert (
        _get_bsz(output.logits)
        == _get_bsz(output.target_label_values)
    ...
    flat_logits, flat_targets, flat_bboxes = (
        _flatten(output.logits),
        _flatten(output.target_label_values),
        _flatten(output.word_bboxes),
    )
    ...
    return flat_logits.unsqueeze(0), flat_targets.unsqueeze(0), flat_bboxes.unsqueeze(0)
