...
def extract_field_validation(model: BaseModel) -> dict:
    out = {}
    ...
    for name, field in model.__class__.model_fields.items():
        ...
        # fallback for json_schema_extra
...
def inject_validation_fields(node: dict, validation_map: dict):
    if not isinstance(node, dict):
        return

    for key, value in list(node.items()):
        if isinstance(value, dict):
            if key in validation_map and "_target_" not in value:
                node[key] = {
                    "value": value,
                    "_meta": {
                ...
                inject_validation_fields(
                    node[key]["value"], validation_map.get(key, {})
                )

            else:
                inject_validation_fields(value, validation_map.get(key, {}))
...
def to_instantiable_dict(obj: BaseModel, include_schema: bool = False) -> dict:
    """
    ...
    def collect(current_obj, path=""):
        if isinstance(current_obj, BaseModel):
            ...
            if include_schema:
                validation_map = extract_field_validation(current_obj)
                inject_validation_fields(current_dict, validation_map)

            # Traverse fields
            for field_name, field_value in current_obj.__dict__.items():
                field_path = f"{path}.{field_name}" if path else field_name
                collect(field_value, field_path)
    ...
    def assign_targets(current_data, path=""):
        if path in targets:
            if isinstance(current_data, dict):
                ...
                if include_schema:
                    current_data["_validation_"] = validation_schemas.get(path, {})
